...
export interface ApprovalRequestFields {
  ...
  txType?: "MI" | "IC" | "MS";
  /**
   * 암호화 여부 필드 값 — **기본은 공백**이다.
   *
   * "A"(ACK & EOT 전송 안함)를 보내면 **승인 직후 자동으로 망취소된다.** KSnCAT 이 승인
   * 결과를 호스트에 전달하고 확인을 받지 못하면 전달 실패로 보고 되돌리기 때문이다.
   * 2026-09-15 현장에서 "A" 일 때 매번 취소되고, 공백으로 바꾸자 승인이 유지되는 것을 확인했다.
   *
...
export function buildApprovalTelegram(f: ApprovalRequestFields): Buffer {
  const body = Buffer.concat([
    ...
    */
    fixed(f.kioskMode ? "K" : (f.encryptFlag ?? ""), 1), // 암호화 여부
